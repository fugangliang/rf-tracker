...
**9/9 9:44 の無人実行はGarminログイン失敗（Failed to retrieve social profile）で全工程が止まった**（8/20確定の
一時不調パターン。無人実行2回中2回失敗＝9/8はgh不在・9/9はGarmin側）。14:50に同一トークンで手動再実行し成功:
写し233件（9/8〜9/9更新）・アドバイス2026-09-09生成・push commit 656a519・iCloud `garmin_20260909.json`。
手動実行の出力はstdoutのみで `auto_fetch.log` には残らない（launchd経由のみ記録）。
- **v1.6.2: 再試行を実装（RF承認2026-09-09・8/20提案の採用）**。`garmin_fetch.py` に `retry()` を追加し、
  Garminログイン失敗は3分→7分待ちで最大3回（`LOGIN_RETRY_WAITS`）、同期push失敗は30秒→90秒で最大3回
  （`PUSH_RETRY_WAITS`）試みる。全滅時は従来どおり exit 0（ログに「再試行N回とも失敗」）。
  待ちを無効化した単体検証（2回失敗→3回目成功・全滅→例外伝播）と `--stdout` 通し実行で確認済み。
  **初の無人実行は9/10 9:30 → `auto_fetch.log` で再試行ログの有無を確認**
- **v1.6.2: 基礎代謝設定＋目安摂取kcal（RF指摘「目標摂取カロリーが高すぎる。オムロンの基礎代謝は1,900」）**。
  原因=Garminの基礎代謝は固定2,226kcal（kcalOut−kcalActiveが毎日一定）で体組成計より約330高く、
  収支の赤字が過大・アドバイスの食事目安も高止まり・Garminアプリの摂取目標2,384も同根。
  対処: ①`logic.js energyOut()`＝bmr設定時は「基礎代謝＋Garmin活動kcal」、未設定はGarmin総消費（既定）。
  `energy` に basis/basisLabel/intakeTarget（=7日平均消費−目標赤字・消費n≥3で摂取記録なしでも出す）を追加
  ②保全タブ設定に「基礎代謝」（meta `bmr`）・収支カードに「目安摂取 kcal/日」 ③Mac側アドバイス生成は
  端末設定を読めないため `data/advice_config.json` の `bmr`（=1900・push禁止領域）を使い、文脈に
  【消費kcalの算定根拠】節を追加・INSTRUCTIONSで「合計kcalは摂取目安を上限、Garmin目標から独自計算しない」
  ④仕様書§4.7・F7追記 ⑤テスト7項目追加=計125。9/9 15:03に新ロジックでアドバイス再生成・push済み
  （食事目安 1,800〜1,900 → 上限1,683kcal）。
  - **RF側**: iPhoneでv1.6.2に更新後、保全タブ「基礎代謝」に1900を入れて保存（未設定だと従来のGarmin推定のまま）。
    Garmin Connectアプリの摂取目標も手動で約1,700に下げると食事ログ画面が実態に合う
  - 基礎代謝は体組成計の推定値であり真値ではない。主判定は従来どおり体重の28日勾配（収支は補助軸）
- omronGaps は 8/27・8/28 を再検出（9/8に追補済み・状態未記録のため。実害なし）
...
- **v1.6.1（2026-09-08未明）: RF「アドバイスの内容が薄い」→深掘り化**。①400字制限を900〜1300字・各節2〜4文＋
  「傾向」「今日の行動計画（朝/日中/夜）」に拡張 ②材料追加: 就寝時刻 `bedHour`（Garmin sleepStartTimestampLocal・
  0:30=24.5）・睡眠時間 `sleepHrs`（当日分も配信）、前日の食事内訳（`data/nutrition/YYYY-MM-DD.json`・Macローカルのみ・
  食事ごとのkcal/PFC/品名） ③文脈に14日の傾き・交絡別・曜日別・就寝時刻別（24時前後）の比較・体組成28日履歴を追加
  （`advice_context.js`）。テスト118項目
- **2026-09-08 9:30 初回本番**: 取得・アドバイス生成は成功、pushが `gh` 不在で失敗（launchdのPATHに
  /opt/homebrew/bin が無い）→ `sync_push.py` を絶対パス `GH_BIN` に修正し手動再実行で配信済み。
  launchdから呼ぶ外部コマンドは常に絶対パス（claude/node/ghすべて対応済み）。
  同日、OMRON取りこぼし（8/26〜28）を `--since 2026-08-25` で追補。RFが体重測定を再開（9/8 89.6kg）
...
- ~~提案中（RF判断待ち）: ログイン失敗時のスクリプト内リトライ~~ → 2026-09-09 v1.6.2で実装済み
