...
- `garmin_fetch.py`: `get_user_summary` から活動量を取得。**当日分は常にnull**（9:30時点は部分値）、
  翌日以降のローリング再配信で確定。kcalIn/proteinは常にnull（手入力保全）
...
- RF側アクション: ①iPhoneでアプリを2回開き直しv1.4.0（7タブ）を確認 ②`EXTRACTION_PROMPT.md` の
  改訂版をclaude.ai抽出プロジェクトのカスタム指示に貼り直す ③**体重測定を再開**（8/18以降ゼロ。
  週2回以上・9:30前にオムロンコネクトへ転送）。測定がないと減量タブは「判定保留」のまま
