...
- **未決（RF裁定待ち）: 「アプリ内で常時最新」＝毎朝の「ファイルから取込」タップの廃止**。提案は
  Mac側の写し（最新エクスポート＋日次自動データ累積）をAES-256-GCMで暗号化して `docs/data.enc` として
  自動commit/push、アプリが起動時に取得・復号・マージ取込（鍵は保全タブに1回入力）。要件§6「外部送信ゼロ」の
  例外になるため承認が必要。無人push可能なことは確認済み（osxkeychain・dry-run成功）
...
1. `docs/` を編集 → `node test/acceptance.js`（96項目、data/があれば受け入れ含む）
